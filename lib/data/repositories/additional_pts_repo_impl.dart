...
class AdditionalPointsRepositoryImpl implements AdditionalPointsRepository {
  ...
  @override
  Future<Either<Failure, int>> getPointsExchange() async {
    if (await _networkInfo.isConnected) {
      try {
        final account = await _localDataSource.getCachedAccount();
        final remoteResponse = await _additionalPointsRemoteDataSource
            .getExchange(account!.token!);
        return Right(remoteResponse);
      } on ServerException catch (e) {
        return Left(ServerFailure(message: e.message));
      } on UpdateException catch (e) {
  ...
  @override
  Future<Either<Failure, Unit>> setPointsExchange(int price) async {
    if (await _networkInfo.isConnected) {
      try {
        final account = await _localDataSource.getCachedAccount();
        final remoteResponse = await _additionalPointsRemoteDataSource
            .setExchange(price, account!.token!);
        return Right(remoteResponse);
      } on ServerException catch (e) {
        return Left(ServerFailure(message: e.message));
      } on UpdateException catch (e) {
  ...
  Future<Either<Failure, Unit>> addEachAdditionalPoints(
      List<AdditionalPoints> additionalPoint) async {
    if (await _networkInfo.isConnected) {
      try {
        final account = await _localDataSource.getCachedAccount();
        final remoteResponse = await _additionalPointsRemoteDataSource
            .addEachAdditionalPoints(additionalPoint, account!.token!);
        return Right(remoteResponse);
      } on ServerException catch (e) {
        return Left(ServerFailure(message: e.message));
      } on UpdateException catch (e) {
