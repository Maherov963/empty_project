    ...
    if (_screenSize != updatedScreenSize) {
      ...
      setState(() {
        ...
        final double viewWidth = clampDouble(
            anchorRect.width, constraints.minWidth, constraints.maxWidth);
        final double viewHeight = clampDouble(_screenSize!.height * 2 / 3,
            constraints.minHeight, constraints.maxHeight);
        final Size updatedViewSize = Size(viewWidth, viewHeight);

        switch (Directionality.of(context)) {
          case TextDirection.ltr:
            final double viewLeftToScreenRight =
                _screenSize!.width - anchorRect.left;
            final double viewTopToScreenBottom =
                _screenSize!.height - anchorRect.top;
            ...
            if (viewLeftToScreenRight < viewWidth) {
              topLeft = Offset(
                  _screenSize!.width - math.min(viewWidth, _screenSize!.width),
                  anchorRect.top);
            }
            if (viewTopToScreenBottom < viewHeight) {
              topLeft = Offset(
                  topLeft.dx,
                  _screenSize!.height -
                      math.min(viewHeight, _screenSize!.height));
            }
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                Expanded(
                  ...
                  padding: effectivePadding,
                  child: IgnorePointer(
                    ignoring: !widget.searchController.isOpen,
                    child: TextField(
                      focusNode: _focusNode,
